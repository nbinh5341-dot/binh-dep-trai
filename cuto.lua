...
-- HÀM TẠO MÀN ĐEN CHE GIAO DIỆN CHO MOBILE
...
local function runBloxFruitsMacro()
    -- Đợi 10 giây ban đầu trước khi bật màn đen và chạy click
    task.wait(10)

    createMobileBlackScreen()

    while true do
        clickPercent(0.107, 0.720) -- Bước 1
        task.wait(0.8)

        clickPercent(0.195, 0.655) -- Bước 2
        task.wait(1.2)

        clickPercent(0.262, 0.380) -- Bước 3
        task.wait(0.8)

        clickPercent(0.355, 0.455) -- Bước 4
        task.wait(0.6)

        clickPercent(0.900, 0.720) -- Bước 5
        task.wait(0.8)

        -- (Bước 6 cũ ở vị trí này đã được xoá bỏ hoàn toàn)

        clickPercent(0.107, 0.095) -- Bước 6 mới (Trước là bước 7)
        task.wait(1.2)

        clickPercent(0.500, 0.360) -- Bước 7 mới (Trước là bước 8)
        task.wait(0.8)

        clickPercent(0.370, 0.650) -- Bước 8 mới (Trước là bước 9)
